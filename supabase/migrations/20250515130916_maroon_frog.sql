/*
  # Add Permission Hierarchy and Error Handling

  1. Changes
    - Create permission error logs table
    - Add functions for permission validation and error logging
    - Update RLS policies to use permission checks
    - Add error handling for role-based operations

  2. Security
    - Enable RLS on error logs
    - Add policies for system logging and user access
...
RETURNS trigger AS $$
BEGIN
  IF NOT check_role_permission(auth.uid(), 'provider', 'access') THEN
